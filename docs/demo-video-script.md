...
| 0:45–1:05 | In Bob IDE, ask Bob to launch `python -m vesper workspace`; confirm R3 and click Run verification. Show genuine progress and logs. Explain that this is a seeded defect with an existing corrected candidate. |
| 1:05–1:30 | Open the workspace Execution tab and show both original execution records: expiry day is inclusive; expected 900, observed 1000. |
| 1:30–1:50 | Open the workspace Patch tab, then the full report for side-by-side context and snapshot hashes. The accepted test remains unchanged. |
...
| 2:20–2:45 | Explain limits: supplied demo only, correct requirements still matter, no measured time savings, six-task Codex pilot found no additional errors and exposed a false block, since fixed. |
...
The local workspace uses React. It invokes the existing Python/Maven verifier; it does not execute a Bob model in the browser. Show the actual Bob launch/interpretation session to demonstrate Bob use. No real human approval should be recorded solely to stage the video.
